...
namespace Log4Qt
{
    ...
    QString WriterAppender::encodeName() const
    {
        QMutexLocker locker(&mObjectGuard);
        return mpEncoding ?  QString(mpEncoding->name()) : "";
    }

    void WriterAppender::setEncodeName(const QString& name)
    {
        setEncoding(QTextCodec::codecForName(name.toLatin1()));
    }
